...
{
    public class JobsControllerTests(CustomWebApplicationFactory factory)
    : IntegrationTestBase(factory)
    {
        ...
        {
            var job = await SeedJobAsync(Factory.AuthContext.CurrentUserId, ct: TestContext.Current.CancellationToken);
        ...
        public async Task GetAll_ShouldReturnOnlyJobsBelongingToCurrentUser()
        {
            ...
            var response = await Client.GetAsync("/api/jobs", TestContext.Current.CancellationToken);
        ...
        {
            Factory.AuthContext.CurrentUserId = $"user-with-no-jobs-{Guid.NewGuid()}";
